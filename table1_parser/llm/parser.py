...
def _heuristic_interpretation(
    ...
    """Build a compact structured view of pre-LLM heuristic interpretation."""
    return {
...
def _diff_interpretations(heuristics: dict[str, Any], result: LLMTableInterpretation) -> str:
    """Create a concise human-readable diff between heuristic and LLM interpretations."""
    lines: list[str] = []
    ...
    for row_start, final_var in sorted(final_vars.items()):
        ...
        if heuristic_var["row_end"] != final_var.row_end:
            lines.append(
                f"variable rows changed at {row_start}: {heuristic_var['row_end']} -> {final_var.row_end}"
            )
        if heuristic_var["variable_type"] != final_var.variable_type:
            lines.append(
                f"variable type changed at {row_start}: {heuristic_var['variable_type']} -> {final_var.variable_type}"
            )
        ...
        if heuristic_levels != final_levels:
            lines.append(
                f"variable levels changed at {row_start}: {heuristic_levels} -> {final_levels}"
            )
    ...
    for col_idx, final_col in sorted(final_columns.items()):
        ...
        if heuristic_col["inferred_role"] != final_col.inferred_role:
            lines.append(
                f"column role changed at {col_idx}: {heuristic_col['inferred_role']} -> {final_col.inferred_role}"
            )
...
def _write_trace_artifacts(
    *,
    ...
    heuristics = _heuristic_interpretation(table, row_classifications, variable_blocks, column_roles)
    _write_json(
    ...
    (trace_path / "diff.txt").write_text(
        f"timestamp: {timestamp}\n" + _diff_interpretations(heuristics, result) + "\n",
        encoding="utf-8",
    )
